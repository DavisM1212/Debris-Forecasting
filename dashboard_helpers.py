...
def select_year_index(years, year_choice):
...
def summarize_paths_at_year(years, paths, year_choice):
...
def growth_rate_annualized(base_value, current_value, years_elapsed):
...
def format_musd(value, decimals=1):
...
def format_pct(value, decimals=1, show_sign=False):
    if value is None or not np.isfinite(value):
        return "–"
    pct_val = value * 100
    sign = ""
